...
type BrandIconProps = { size?: number; strokeWidth?: number; "aria-hidden"?: boolean };

function brandIcon(path: string) {
  return function BrandIcon({ size = 18, "aria-hidden": ariaHidden }: BrandIconProps) {
    return (
      <svg
        width={size}
        height={size}
...
const ICONS: Record<SocialKind, React.ComponentType<BrandIconProps>> = {
  // Lucide removed brand-named icons (Linkedin/Instagram/Twitter) over
  // trademark/licensing concerns, so each platform mark is inlined here
  // as its own filled brand SVG (path data from simple-icons) instead of
  // a generic stand-in — generic icons made LinkedIn and WhatsApp
  // (both near-identical chat bubbles) indistinguishable from each other.
