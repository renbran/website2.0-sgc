...
/**
 * AwardTrophy — credential badge with laurel wreath matching the
 * eiger-marvel-hr-plat reference design exactly.
 *
 *   Two detailed SVG laurel branches curving up from the bottom,
 *   gold gradient fill, framing centered text on a dark backdrop:
 *
 *     [GOLD pill]
 *     HEADLINE
 *     --- hairline ---
 ...
 *     Date (bold)
 *
 * SVG paths extracted from eiger src/components/ui/award.tsx.
 * viewBox: 0 0 892.77 688.08, gradient: #F5D98A → #E8C36A → #C9A14A.
 */
...
function cn(...args: CnArg[]): string {
  return args
    ...
    .map((a) => (typeof a === "string" || typeof a === "number" ? String(a) : ""))
    .join(" ");
}
...
/**
 * The detailed laurel wreath SVG paths extracted from eiger.
 * Right branch: M892.6,358.7... (4290 chars)
 * Left branch: M122.6,347.7... (4161 chars)
...
export interface AwardTrophyProps {
  /** The "level" tag rendered in the colored pill (e.g. "GOLD"). */
  level: string;
  /** Top-line bold headline (e.g. "25+ Years"). */
  headline: string;
  /** Subline description. */
  subtitle: string;
  /** Recipient line in italic. */
  recipient?: string;
...
export function AwardTrophy({
  ...
  // when the marquee renders the same credential content twice back-to-back.
  const gradientId = `laurelGold-${useId().replace(/:/g, "")}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path fill={`url(#${gradientId})`} d={LEFT_BRANCH} />
      </svg>

      {/* Text container — z-10 above SVG, centered, constrained width */}
